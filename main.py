...
from scripts.logger import log_info, log_ok, log_warning, log_error, monitor_log, backup_log, drift_log, deploy_log, telegram_log
...
def run():
    ...
    for s in status_list:
        if s['up']:
            monitor_log("  {} ({}) -- UP".format(s['name'], s['ip']))
    ...
    for d in DEVICES:
        ...
        if ok:
            backup_log("  {} -- OK -- {} bytes".format(d['name'], size))
    ...
    for r in drift_results:
        if r['changed']:
            ...
            drift_log("  {} -- Khong co thay doi".format(r['name']))
